...
export default function ExamProfile(props) {
  ...
  if (examScore && examScore.length > 0) {
    totalScore = examScore.map(score => {
      ...
      const subject = Object.keys(examLength).sort().map((sub, ind) => (ind === 3 ? <span key={sub}>{subjectList[sub]}</span> : <span key={sub}>{`${subjectList[sub]}, `}</span>));
    ...
    totalScore = (<div className="card-body">No exam record!</div>);
  }
